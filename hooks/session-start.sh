...
REMINDERS=$(sqlite3 -separator '|' "$DB" "SELECT id, summary, session_id, repo_path FROM reminders WHERE status = 'active' ORDER BY created_at DESC LIMIT 10;" 2>/dev/null || true)

[ -z "$REMINDERS" ] && exit 0

echo "POKE: Active reminders:"
